...
```{r, echo=FALSE}
food <- matrix(c(47, 0, 0,
                 0, 12, 21), nrow = 2, byrow = TRUE)
dimnames(food) <- list(c("Vegan", "Not-Vegan"), c("Soy", "Milk", "Meat"))
food
```
...
Music <- matrix(
  c(151, 130, 12, 7,
    ...
    0, 4, 2, 165),
  byrow = TRUE, nrow = 3)
dimnames(Music) <- list(c("Psych", "Econ", "Law"),
...
RCT <- matrix(
  c(71, 30,
    50, 100),
  nrow = 2, byrow = TRUE)
dimnames(RCT) <- list(c("Sick", "Recovered"),
                      c("Treatment", "Control"))
RCT
...
BFX <- contingencyTableBF(RCT, sampleType = "jointMulti")
...
Performance <- matrix(
  c(794, 86,
    150, 570),
  nrow = 2, byrow = TRUE)
dimnames(Performance) <- list(c("Approve", "Disapprove"),
