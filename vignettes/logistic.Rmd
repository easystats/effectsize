...
Let's compute now, a traditional Cohen's *d* using the `effectsize` package. While this *d* should be close to 1, it should theorethically be a tiny bit larger, because it takes into account the (pooled) SD of the whole variable `x` (accross the groups), which because of the difference is a bit larger than 1.
...
As expected, it's pretty close to 1 times the SD of the sample. Interestingly, one can estimate the Cohen's *d* directly from the result of the *t*-test, using the *t* statistic. We can convert it to a *d* using the `effectsize` pacakge:
...
Another way of investigating these differences is through the lense of a logistic regression. The main difference is that here, the group variable `y` becomes the outcome and `x` the predictor. Let's fit such model and investigate the parameters: 
...
How to interepret this output? The coefficients of a logistic model are expressed in log-odds, which is a metric of probability. Using the [**modelbased**](https://github.com/easystats/modelbased) pacakge, one can easilly visualize this model:
