...
Standardizing the coefficient of this simple linear regression gives a value of `0.87`, but did you know that this is actually the **same as a correlation**? (Technically a *partial* correlation when multiple predictors are present). Thus, you can eventually apply some (*in*)famous interpretation guidelines (e.g., Cohen's rules of thumb).
...
Now, let's apply another method to obtain effect sizes for frequentist regressions, based on the statistic values. We will convert the *t*-value (and its degrees of freedom, *df*) into a correlation coefficient *r*.
...
Wow, the retrieved correlations coefficients from the regression model are **exactly** the same as the partial correlations. Can we arrive to the same conclusions by standardizing the parameters?
