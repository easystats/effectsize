...
knitr::kable(parameters_standardize(model, method="refit"), digits=2)
```
...
```
```{r message=FALSE, warning=FALSE, echo=FALSE, eval = FALSE}
knitr::kable(parameters_standardize(model, method="refit", robust=TRUE), digits=2)
```
...
```
```{r message=FALSE, warning=FALSE, echo=FALSE, eval = FALSE}
knitr::kable(parameters_standardize(model, method="smart"), digits=2)
```
...
comparison <- function(model, robust=FALSE){
  out <- parameters_standardize(model, method="refit", robust=robust)[1:2]
  
  out$smart <- tryCatch({
    out[, 2] - parameters_standardize(model, method="smart", robust=robust)[, 2]
}, error = function(error_condition) {
    "Error"
})
  out$classic <- tryCatch({
    out[, 2] - parameters_standardize(model, method="classic", robust=robust)[, 2]
}, error = function(error_condition) {
    "Error"
})
...
```{r message=FALSE, warning=FALSE, echo=FALSE, eval = FALSE}
library(rstanarm)
model <- stan_glm(Sepal.Length ~ Petal.Width + Group_Sepal.Width, data=data, refresh = 0)
knitr::kable(comparison(model), digits=2)
```
