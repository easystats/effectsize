...
In both theoretical and applied research, it is often of interest to assess the strength of an observed association. Beyond their use in interpreting the results from a given study, they can be aggregated in meta-analyses [@lakens2013calculating]. We review here the most common effect sizes for analyses of categorical variables that use the $\chi^2$ (chi-square) statistic, and introduce a new one---`r "\U05E4"` (Fei)---alongside the `{effectsize}` package [@benshachar2020effectsize] in the R programming language [@base2023], which implements these measures and their confidence intervals.

<!-- RT: Seems like the special character פ is not supported in the resulting PDF. It's unicode counterpart, "\U05E4", works in the R console, but unfortunately not in the PDF conversion either. According to Chat GPT, this is because the default font (Computer Modern) does not support Hebrew characters, so I changed the font to Times New Roman (for now) and that seems to work (with the unicode). -->

<!-- MSB: This actually might just be the abstract? RT: Perhaps, but based on other papers in that journal, we also need a separate intro. -->
...
Tschuprow [@tschuprow1939principles] devised an alternative value, at <value><!-- doesn't render -->, which is 1 only when the columns are completely dependent on the rows *and* the rows are completely dependent on the columns, which is only possible when the contingency table is a square.
...
We present here a new effect size, `r "\U05E4"` (Fei), which normalizes goodness-of-fit $\chi^2$ by the proper $\chi^2_\text{max}$ for non-uniform and/or multinomial variables.
...
<!-- I'm having a hard time understanding this sentence... I attempted to rewrite it for clarity but please check I have not altered the original meaning. -->
