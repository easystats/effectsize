...
In both theoretical and applied research, it is often of interest to assess the strength of an observed association. Beyond their use in interpreting the results from a given study, they can be aggregated in meta-analyses<!-- (ref?)-->. We review here the most common effect sizes for analyses of categorical variables that use the $\chi^2$ (chi-suqare) statistic, and introduce a new one -- $פ$ (Fei)  --alongside the `{effectsize}` package [@benshachar2020effectsize] in the R programming language [@base2023], which implements these measures and their confidence intervals.

<!-- MSB: This actually might just be the abstract? -->
